...
export default function MovieDetailPage() {
  ...
  useEffect(() => {
    ...
    getMovieById(videoId)
      .then(setMovieData)
      .catch((e) => console.error("Failed to load movie:", e));
  }, [videoId]);
  ...
  // ---- Load favorite/watchlist ----
  useEffect(() => {
    if (!user || !videoId) return;
    fetchFavorites(user.id)
      .then((favs) => setIsFav(favs.some((t) => t.youtube_id === trailer.youtube_id || t.id === trailer.id)))
      .catch(console.error);
    fetchWatchlist(user.id)
      .then((wls) => setInList(wls.some((t) => t.youtube_id === trailer.youtube_id || t.id === trailer.id)))
      .catch(console.error);
  ...
  const ensureProfile = async () => {
    ...
    const { data: existing } = await supabase
      ...
      .maybeSingle();
    if (!existing) {
      await supabase.from("profiles").insert({ id: user.id, username: user.email ?? "Unknown" });
    }
  };
  ...
  // ---- Placeholder cast & similar ----
  const cast = Array.from({ length: 8 }).map((_, i) => ({ id: i, name: i % 2 === 0 ? "Reina Holt" : "Aron Vega", role: i % 2 === 0 ? "Main Actor" : "Director" }));
  const similar = Array.from({ length: 8 }).map((_, i) => ({ id: i, title: `Movie ${i + 1}`, poster: "https://via.placeholder.com/150x220?text=Movie" }));
      ...
      <div className="max-w-[1200px] mx-auto p-6 flex gap-8">
        ...
        <div className="w-[360px] flex-shrink-0">
          ...
          <div className="mt-4 flex flex-col gap-2">
            <button onClick={async () => { if (!user) return alert("Sign in"); setIsFav(v=>!v); try{await toggleFavorite(user.id, trailer);}catch{setIsFav(v=>!v);} }} className={`px-4 py-2 rounded font-semibold ${isFav ? "bg-purple-600" : "bg-white/10 border border-white/10 hover:bg-white/15"}`}>{isFav ? "Favorited" : "Add to Favorites"}</button>
            <button onClick={async () => { if (!user) return alert("Sign in"); setInList(v=>!v); try{await toggleWatchlist(user.id, trailer);}catch{setInList(v=>!v);} }} className={`px-4 py-2 rounded font-semibold ${inList ? "bg-indigo-600" : "bg-white/10 border border-white/10 hover:bg-white/15"}`}>{inList ? "In Watchlist" : "Add to Watchlist"}</button>
          </div>
        </div>
        ...
        <div className="flex-1">
          ...
          <section className="max-w-3xl mx-auto mb-10">
            ...
            <form className="flex gap-2 mb-4 bg-white/5 p-3 rounded-lg border border-white/10" onSubmit={async e => {
              e.preventDefault();
              if (!user) return alert("Sign in to comment");
              const content = newComment.trim(); if (!content) return;
              const optimistic: DisplayComment = { id: `temp-${Date.now()}`, user_id: user.id, trailer_id: trailer.id, content, created_at: new Date().toISOString(), likes: 0, commenterUsername: user.email ?? user.id, _unsynced: true, _error: null };
              setComments(prev => [optimistic, ...prev]);
              setNewComment("");
              try {
                const saved = await addComment(user.id, videoId, content);
                setComments(prev => [saved, ...prev.filter(c => !c.id.startsWith("temp-"))]);
              } catch (err) {
                setComments(prev => prev.map(c => c.id === optimistic.id ? { ...c, _unsynced:true, _error: err instanceof Error ? err.message : "Failed" } : c));
              }
            }}>
              <input value={newComment} onChange={e=>setNewComment(e.target.value)} placeholder="Write a comment..." className="flex-1 px-4 py-2 rounded-lg text-black focus:outline-none focus:ring-2 focus:ring-purple-500 bg-gray-100"/>
              <button type="submit" className="bg-purple-600 hover:bg-purple-700 px-5 py-2 rounded-lg font-semibold text-white">Post</button>
            </form>
            ...
            <div className="space-y-3">
              {comments.map(c => (
                <div key={c.id} className="bg-white/5 border border-white/10 p-4 rounded-lg">
                  ...
                  <button className="mt-2 text-xs bg-pink-700 hover:bg-pink-600 px-3 py-1 rounded font-semibold text-white" onClick={async () => {
            ...
            <div className="flex gap-4 overflow-x-auto">
              {similar.map(m => (
                <div key={m.id} className="text-sm">
                  <img src={m.poster} alt={m.title} className="w-20 h-28 rounded" />
