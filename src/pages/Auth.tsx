...
export default function Auth({ mode: initialMode = 'login' as Mode }) {
  ...
  return (
    <main className="max-w-[420px] mx-auto my-8 p-4">
      <div className="bg-surface rounded-md p-4 shadow-xl">
        ...
        <form className="grid gap-3" onSubmit={(e) => e.preventDefault()}>
          ...
          <button className="px-4 py-3 rounded-sm bg-button text-[#1c1530] font-bold">{mode === 'login' ? 'Login' : mode === 'signup' ? 'Create Account' : 'Send Reset Link'}</button>
        </form>
        <div className="flex gap-2 mt-3 text-muted">
